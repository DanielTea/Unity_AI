...
function Experience:store(reward, state, terminal, action)
 ...
 -- log.info('Experience replay, index count '..self.index)
 -- log.info('Experience replay, capacity count '..self.capacity)
...
function Experience:sample()
  ...
  elseif self.memPriority == 'rank' then
    ...
    local distIndex = math.floor(N / self.capacity * 100) --wichtig hier alloc memory abhängig von size
